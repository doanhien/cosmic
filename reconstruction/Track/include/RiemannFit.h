#ifndef _RIEMANN_FIT_H_
#define _RIEMANN_FIT_H_
...
class RiemannFit{
 public:
  ...
  double GetRadius() { return rc; }

  vector<double> GetCircleParameters() {
    vector<double> param;
    param.push_back(xc);
    param.push_back(yc);
    param.push_back(rc);
 ...
 private:
  vector<RiemannHit_t*>hits;
  vector<RiemannHit_t*>projections;
  DMatrix *CovR_;
  DMatrix *CovRPhi_;
