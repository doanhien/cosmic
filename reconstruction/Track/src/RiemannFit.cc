...
bool RiemannFit_hit_cmp(RiemannHit_t *a,RiemannHit_t *b){
  return (a->z>b->z);
}

/// Add a hit to the list of hits using cylindrical coordinates
jerror_t RiemannFit::AddHit(double r, double phi, double z)
{
...
jerror_t RiemannFit::AddHitXYZ(double x,double y, double z){
  RiemannHit_t *hit = new RiemannHit_t;
  hit->x = x;
  hit->y = y;
  hit->z = z;
  hit->covx=1.;
...
/// Add a hit to the list of hits using Cartesian coordinates
jerror_t RiemannFit::AddHit(double x,double y, double z,double covx,
...
// Fit sequence
jerror_t RiemannFit::DoFit(double rc_input){
...
jerror_t RiemannFit::CalcNormal(DMatrix A,double lambda,DMatrix &N){
  double sum=0;
...
jerror_t RiemannFit::FitCircle(double rc){
  // Covariance matrices
...
//
jerror_t RiemannFit::FitCircle(){  
  if (hits.size()==0) return RESOURCE_UNAVAILABLE;
  std::cout << "\n doing RiemannFit for circle" << std::endl;
  ...
  for (unsigned int i=0;i<hits.size();i++){
    ...
    cout << " X0: " << X(i,0) << ", X1: " << X(i,1) << ", X2: " << X(i,2) << endl;
  }
  ...
  std::cout << "B2 = " << B2 << ", B1: " << B1 << ", B0: " << B0 << std::endl;
  ...
  std::cout << "smallest eigen values: " << lambda_min << std::endl;
  ...
  std::cout << "center of circle, x0: " << xc << ", y0: " << yc << ", r = " << rc << "\n " << std::endl;

  return NOERROR;
}

// Charge-finding routine with corrected CRPhi (see above)
double RiemannFit::GetCharge(double rc_input){
  // Covariance matrices
  DMatrix CRPhi(hits.size(),hits.size());
  DMatrix CR(hits.size(),hits.size());
  // Auxiliary matrices for correcting for non-normal track incidence to FDC
...
jerror_t RiemannFit::FitLine(){
  ...
  for (unsigned int m=0;m<hits.size();m++){
    ...
    RiemannHit_t *temphit = new RiemannHit_t;
    temphit->z=hits[m]->z;
